    ...
    private fun submitProduct() {
        ...
        viewModelScope.launch {
            ...
            when (apiResult) {
                ...
                is Result.Error -> {
                    // Cache the product locally in Room if submission failed (e.g. offline/network failure)
                    val draftId = "draft-${System.currentTimeMillis()}"
                    val draftItem = com.agriflow.app.features.MyStore.StoreInventoryEntity(
                        id = draftId,
                        productName = currentState.name,
                        description = currentState.description,
                        price = parsedPrice ?: 0.0,
                        ...
                        unit = currentState.selectedUnit,
                        categoryId = currentState.selectedCategoryId ?: "",
                        syncStatus = com.agriflow.app.core.database.SyncState.PENDING,
                        createdAt = System.currentTimeMillis(),
                        imageUrls = uploadedUrls.joinToString(",")
                    )
                    marketplaceRepository.saveStoreInventoryItem(draftItem)

                    _events.send(AddProductEvent.ShowSnackbar("Offline mode: Product saved locally as draft and will sync later."))
                    _events.send(AddProductEvent.MapsBack)
                }
            }
        }
